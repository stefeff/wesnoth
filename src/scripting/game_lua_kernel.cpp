...
int game_lua_kernel::impl_game_config_get(lua_State *L)
{
	...
	if(classification.type==campaign_type::type::multiplayer) {
		...
		return_cfgref_attrib_deprecated("era", "wesnoth.game_config", INDEFINITE, "1.17", "Use wesnoth.scenario.era instead",
			game_config_manager::get()->game_config().find_mandatory_child(str_era,str_id,classification.era_id));
...
int game_lua_kernel::impl_scenario_set(lua_State *L)
{
	...
	if(strcmp(m, "end_level_data") == 0) {
		...
		data.proceed_to_next_level = cfg[str_proceed_to_next_level].to_bool(true);
		data.transient.carryover_report = cfg[str_carryover_report].to_bool(true);
		data.prescenario_save = cfg[str_save].to_bool(true);
		data.replay_save = cfg[str_replay_save].to_bool(true);
		data.transient.linger_mode = cfg[str_linger_mode].to_bool(true) && !teams().empty();
		data.transient.reveal_map = cfg[str_reveal_map].to_bool(true);
		data.is_victory = cfg[str_result] == level_result::victory;
		data.test_result = cfg[str_test_result].str();
...
int game_lua_kernel::intf_find_reach(lua_State *L)
{
	...
	static const std::vector<std::string> table_config{"x", "y", "moves_left"};
	...
	for (int i = 0; i < nb; ++i)
	{
		...
		luaW_push_namedtuple(L, table_config, "mt_loc_move");
...
int game_lua_kernel::intf_find_vision_range(lua_State *L)
{
	...
	static const std::vector<std::string> table_config{"x", "y", "vision_left"};

	lua_createtable(L, res.destinations.size() + res.edges.size(), 0);
	for(const auto& d : res.destinations) {
		luaW_push_namedtuple(L, table_config, "mt_loc_vision");
...
int game_lua_kernel::intf_find_cost_map(lua_State *L)
{
	...
	// create return value
	static const std::vector<std::string> table_config{"x", "y", "cost", "reach"};

	lua_createtable(L, location_set.size(), 0);
	int counter = 1;
	for (const map_location& loc : location_set)
	{
		luaW_push_namedtuple(L, table_config, "mt_loc_cost");
...
int game_lua_kernel::intf_put_unit(lua_State *L)
{
	...
	if((luaW_isunit(L, 1))) {
		...
		if (!map().on_board(loc)) {
			loc.set_wml_x(cfg[str_x]);
			loc.set_wml_y(cfg[str_y]);
			if (!map().on_board(loc))
				return luaL_argerror(L, 2, "invalid location");
		}
...
static int intf_append_ai(lua_State *L)
{
	...
	if(added_dummy_stage) {
		for(auto iter = cfg.ordered_begin(); iter != cfg.ordered_end(); iter++) {
			if(iter->key == "stage" && iter->cfg[str_name] == "empty") {
				iter = cfg.erase(iter);
			}
		}
	}
...
int game_lua_kernel::intf_add_tile_overlay(lua_State *L)
{
	...
	if (game_display_) {
		game_display_->add_overlay(loc, cfg[str_image], cfg[str_halo],
			team_name, cfg[str_name], cfg[str_visible_in_fog].to_bool(true),
			cfg[str_submerge].to_double(0), cfg[str_z_order].to_double(0));
	}
	return 0;
}
...
void game_lua_kernel::save_game(config &cfg)
{
	...
	for (;;)
	{
		...
		if (is_handled_file_tag(i->key))
		{
			 ...
			 */
			const std::string m = "Tag is already used: [str_ + i->key + ]";
			log_error(m.c_str());
			v.erase(i);
			continue;
		}
