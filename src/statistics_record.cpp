...
static stats_t::battle_result_map read_battle_result_map(const config& cfg)
{
	...
	for(const config& i : cfg.child_range(str_sequence)) {
		...
		item.remove_attribute(str__num);
...
{
	stats_t::hitrate_map m;
	for(const config& i : cfg.child_range(str_hitrate_map_entry)) {
		m.emplace(i[str_cth], stats_t::hitrate_t(i.mandatory_child(str_stats)));
	}
	return m;
}
