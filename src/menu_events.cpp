		...
		events::chat_handler::MESSAGE_TYPE type)
{
	...
	plugins_manager::get()->notify_event("chat",
		config {
			str_sender, preferences::login(),
			str_message, message,
			str_whisper, type == events::chat_handler::MESSAGE_PRIVATE,
		}
	);
}
...
void menu_handler::send_chat_message(const std::string& message, bool allies_only)
{
	...
	if(private_message) {
		if(board().is_observer()) {
			...
			cfg[str_to_sides] = pc_.get_teams()[gui_->viewing_team()].allied_human_teams();
		}
	}

	resources::recorder->speak(cfg);

	add_chat_message(time, cfg[str_id], side, message,
	...
	if(side < 1 || side > menu_handler_.pc_.get_teams().size()) {
		...
		if(action == "on") {
			...
			if(!is_human || !is_droid) {
				...
				changed = true;
				if(is_ai) {
					menu_handler_.pc_.send_to_wesnothd(config {str_change_controller, config {str_side, side, str_player, preferences::login(), str_to, side_controller::human}});
				}
			...
			if(!is_ai || !is_droid) {
				...
				changed = true;
				if(is_human || is_proxy_human) {
					menu_handler_.pc_.send_to_wesnothd(config {str_change_controller, config {str_side, side, str_player, preferences::login(), str_to, side_controller::ai}});
				}
