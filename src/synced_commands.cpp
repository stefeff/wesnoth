...
SYNCED_COMMAND_HANDLER_FUNCTION(attack, child, spectator)
{
	...
	int weapon_num = child[str_weapon].to_int();
...
SYNCED_COMMAND_HANDLER_FUNCTION(move, child, spectator)
{
	...
	bool skip_sighted = child[str_skip_sighted] == "all";
	bool skip_ally_sighted = child[str_skip_sighted] == "only_ally";
...
SYNCED_COMMAND_HANDLER_FUNCTION(custom_command, child, /*spectator*/)
{
	...
	resources::lua_kernel->custom_command(child[str_name], child.child_or_empty("data"));
...
SYNCED_COMMAND_HANDLER_FUNCTION(auto_shroud, child, /*spectator*/)
{
	...
	bool block_undo = child[str_block_undo].to_bool();
...
SYNCED_COMMAND_HANDLER_FUNCTION(debug_teleport, child, /*spectator*/)
{
	...
	const map_location teleport_from(child[str_teleport_from_x].to_int(), child[str_teleport_from_y].to_int(), wml_loc());
	const map_location teleport_to(child[str_teleport_to_x].to_int(), child[str_teleport_to_y].to_int(), wml_loc());
