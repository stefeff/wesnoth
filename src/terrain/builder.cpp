...
void terrain_builder::add_images_from_config(rule_imagelist& images, const config& cfg, bool global, int dx, int dy)
{
	for(const config& img : cfg.child_range("image")) {
		int layer = img[str_layer].to_int();
...

{
	...
	terrain_constraint& constraint = add_constraints(constraints, loc, t_translation::ter_match(cfg[str_type].str(), t_translation::WILDCARD), global_images);
...
void terrain_builder::parse_config(const game_config_view& cfg, bool local)
{
	...
	for(const config& br : cfg.child_range(str_terrain_graphics)) {
		...
		pbr.precedence = br[str_precedence].to_int();
