...
terrain_type::terrain_type(const config& cfg) :
		...
		minimap_image_overlay_(),
		editor_image_(cfg[str_editor_image].empty() ? "terrain/" + minimap_image_ + ".png" : "terrain/" + cfg[str_editor_image].str() + ".png"),
		id_(cfg[str_id]),
		...
		def_type_(),
		union_type_(),
		height_adjust_(cfg[str_unit_height_adjust]),
 ...
 */
	...
	const t_translation::ter_list& vision_alias = t_translation::read_list(cfg[str_vision_alias].str());
	...
	if(village_) {
		income_description_ = cfg[str_income_description];
		if(income_description_.empty()) {
			income_description_ = _("Village");
		}

		income_description_ally_ = cfg[str_income_description_ally];
		if(income_description_ally_.empty()) {
			income_description_ally_ = _("Allied village");
		}

		income_description_enemy_ = cfg[str_income_description_enemy];
		if(income_description_enemy_.empty()) {
			income_description_enemy_ = _("Enemy village");
		}

		income_description_own_ = cfg[str_income_description_own];
