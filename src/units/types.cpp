...
unit_type::unit_type(default_ctor_t, const config& cfg, const std::string & parent_id)
	: cfg_(nullptr)
	, built_cfg_()
	, id_(cfg.has_attribute("id") ? cfg[str_id].str() : parent_id)
	, debug_id_()
		...
		const movement_type_map& mv_types, const race_map& races, const config_array_view& traits)
{
	...
	for(const config& sn : cfg.child_range("special_note")) {
		special_notes_.push_back(sn[str_note].t_str());
	}
	...
	for(const auto [key, cfg] : abilities.all_children_view()) {
		...
		subst_cfg[str_name] = p_ab->substitute_variables(cfg[str_name]);
		subst_cfg[str_female_name] = p_ab->substitute_variables(cfg[str_female_name]);
		subst_cfg[str_description] = p_ab->substitute_variables(cfg[str_description]);
		subst_cfg[str_name_inactive] = p_ab->substitute_variables(cfg[str_name_inactive]);
		subst_cfg[str_female_name_inactive] = p_ab->substitute_variables(cfg[str_female_name_inactive]);
		subst_cfg[str_description_inactive] = p_ab->substitute_variables(cfg[str_description_inactive]);
	...
	for(const config& adv : cfg.child_range("advancement")) {
		for(const config& effect : adv.child_range("effect")) {
			...
			if(!abil_cfg.empty() || effect[str_apply_to] != "new_ability") {
				continue;
			}
...
std::vector<t_string> combine_special_notes(const std::vector<t_string>& direct, const config& abilities, const const_attack_itors& attacks, const movetype& mt)
{
	...
	for(const auto [key, cfg] : abilities.all_children_view()) {
		if(cfg.has_attribute("special_note")) {
			append_special_note(notes, cfg[str_special_note].t_str());
		}
	}
	for(const auto& attack : attacks) {
		for(const auto& p_ab : attack.specials()) {
			if(p_ab->cfg().has_attribute("special_note")) {
				append_special_note(notes, p_ab->cfg()[str_special_note].t_str());
			}
		}
...
int unit_type::resistance_against(const std::string& damage_name, bool attacker) const
{
	...
	for(const auto& p_ab : abilities()) {
		...
		if (!p_ab->cfg()[str_affect_self].to_bool(true)) {
			continue;
		}
...
void unit_type_data::set_config(const game_config_view& cfg)
{
	...
	for(const auto& abil_cfg : cfg.child_range("abilities")) {
		for(const auto [key, child_cfg] : abil_cfg.get().all_children_range()) {
			const std::string& id = child_cfg[str_unique_id].str(child_cfg[str_id]);
	...
	if(get_cfg().has_child("variation")) {
		...
		for (auto& cv : cfg.child_range("variation")){
			for(auto& [variation_id, variation_type] : variations_) {
				if(cv[str_variation_id] == variation_id) {
