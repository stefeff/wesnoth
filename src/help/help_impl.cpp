...
bool section_is_referenced(const std::string &section_id, const config &cfg)
{
	if (auto toplevel = cfg.optional_child(str_toplevel))
	{
		const std::vector<std::string> toplevel_refs
			= utils::quoted_split(toplevel[str_sections]);
	...
	for (const config &section : cfg.child_range(str_section))
	{
		const std::vector<std::string> sections_refd
			= utils::quoted_split(section[str_sections]);
...
bool topic_is_referenced(const std::string &topic_id, const config &cfg)
{
	...
	{
		const std::vector<std::string> toplevel_refs
			= utils::quoted_split(toplevel[str_topics]);
	...
	for (const config &section : cfg.child_range(str_section))
	{
		const std::vector<std::string> topics_refd
			= utils::quoted_split(section[str_topics]);
						   ...
						   section &sec, int level)
{
	if (level > max_section_level) {
		...
		for (it = sections.begin(); it != sections.end(); ++it) {
			if (auto child_cfg = help_cfg->find_child(str_section, str_id, *it))
			{
		...
		generate_topics(sort_generated,(*section_cfg)[str_generator]);

		const std::vector<std::string> topics_id = utils::quoted_split((*section_cfg)[str_topics]);
		std::vector<topic> topics;

		// Find all topics in this section.
		for (it = topics_id.begin(); it != topics_id.end(); ++it) {
			if (auto topic_cfg = help_cfg->find_child(str_topic, str_id, *it))
			{
				std::string text = topic_cfg[str_text];
				text += generate_topic_text(topic_cfg[str_generator], help_cfg, sec, generated_topics);
...
section parse_config(const config *cfg)
{
	section sec;
	if (cfg != nullptr) {
		auto toplevel_cfg = cfg->optional_child(str_toplevel);
...
std::vector<topic> generate_weapon_special_topics(const bool sort_generated)
{
	...
	for (const unit_type_data::unit_type_map::value_type &type_mapping : unit_types.types())
	{
		...
		for(config adv : type.modification_advancements()) {
			for(config effect : adv.child_range(str_effect)) {
				if(effect[str_apply_to] == "new_attack" && effect.has_child(str_specials)) {
					for(config::any_child spec : effect.mandatory_child(str_specials).all_children_range()) {
						if(!spec.cfg[str_name].empty()) {
							special_description.emplace(spec.cfg[str_name].t_str(), spec.cfg[str_description].t_str());
							if(!type.hide_help()) {
								//add a link in the list of units having this special
								std::string type_name = type.type_name();
								//check for variations (walking corpse/soulless etc)
								...
								//so the automatic alphabetic sorting of std::set can use it
								std::string link = make_link(type_name, ref_id);
								special_units[spec.cfg[str_name]].insert(link);
							}
						}
					}
				} else if(effect[str_apply_to] == "attack" && effect.has_child(str_set_specials)) {
					for(config::any_child spec : effect.mandatory_child(str_set_specials).all_children_range()) {
						if(!spec.cfg[str_name].empty()) {
							special_description.emplace(spec.cfg[str_name].t_str(), spec.cfg[str_description].t_str());
							if(!type.hide_help()) {
								//add a link in the list of units having this special
...
std::vector<topic> generate_era_topics(const bool sort_generated, const std::string & era_id)
{
	...
	auto era = game_cfg->find_child(str_era, str_id, era_id);
	if(era && !era[str_hide_help].to_bool()) {
		...
		std::stringstream text;
		text << "<header>text='" << _("Era:") << " " << era[str_name] << "'</header>" << "\n";
		text << "\n";
		...
		topic era_topic(era[str_name], ".." + era_prefix + era[str_id].str(), text.str());
...
std::vector<topic> generate_faction_topics(const config & era, const bool sort_generated)
{
	...
	for (const config &f : era.child_range(str_multiplayer_side)) {
		...
		const std::string ref_id = faction_prefix + era[str_id] + "_" + id;
...
{
	for (const config & era : game_cfg->child_range(str_era)) {
...
{
	auto section_cfg = help_cfg->find_child(str_section, str_id, section_name);
	if (!section_cfg) {
		return std::string();
	}
	...
	std::ostringstream res;
		...
		for (t = topics.begin(); t != topics.end(); ++t) {
			if (auto topic_cfg = help_cfg->find_child(str_topic, str_id, *t)) {
...
void generate_contents()
{
	...
	if (game_cfg != nullptr) {
		const config *help_config = &game_cfg->child_or_empty(str_help);
		try {
			...
			std::stringstream ss;
			for (const config &section : help_config->child_range(str_section))
			{
			...
			hidden_toplevel[str_sections] = ss.str();
			ss.str("");
			for (const config &topic : help_config->child_range(str_topic))
			{
			...
			hidden_toplevel[str_topics] = ss.str();
			config hidden_cfg = *help_config;
			// Change the toplevel to our new, custom built one.
			hidden_cfg.clear_children(str_toplevel);
			hidden_cfg.add_child(str_toplevel, std::move(hidden_toplevel));
