...
void faction_select::on_faction_select()
{
	...
	for(const std::string& leader : flg_manager_.choosable_leaders()) {
		...
		if(unit) {
			...
			leaders.emplace_back(str_label, unit->type_name(), str_icon, icon);
		} else if(leader == "random") {
			leaders.emplace_back(str_label, _("Random"), str_icon, ng::random_enemy_picture);
		} else if(leader == "null") {
			leaders.emplace_back(str_label, font::unicode_em_dash);
		} else {
			leaders.emplace_back(str_label, "?");
		}
	}
