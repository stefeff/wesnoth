...
bool mp_join_game::fetch_game_config()
{
	...
	for(const config& side : get_scenario().child_range(str_side)) {
		...
		if(side[str_controller] == side_controller::reserved && side[str_current_player] == prefs::get().login()) {
			side_choice = &side;
		...
		if(side[str_controller] == side_controller::human && side[str_player_id].empty()) {
			...
			if(side[str_current_player] == prefs::get().login()) {
				side_choice = &side;
				side_num_choice = side_num_counter;
		...
		if(side[str_player_id] == prefs::get().login()) {
...
static std::string generate_user_description(const config& side)
{
	...
	std::string owner = side[str_player_id].str();
...
bool mp_join_game::show_flg_select(int side_num, bool first_time)
{
	if(auto side_choice = get_scenario().optional_child(str_side, side_num - 1)) {
		...
		change[str_name] = prefs::get().login();
...
void mp_join_game::generate_side_list()
{
	...
	for(const auto& side : get_scenario().child_range(str_side)) {
		...
		const std::string color_str = !side[str_color].empty() ? side[str_color] : side[str_side].str();
		const auto team_color_it = game_config::team_rgb_colors.find(color_str);

		if (team_color_it != game_config::team_rgb_colors.end()) {
			item[str_label] = markup::span_color(team_color_it->second[0], side[str_side]);
		} else {
			item[str_label] = side[str_side].str();
		}
		...
		std::string leader_type = leader[str_type];
		std::string leader_gender = leader[str_gender];
		...
		item[str_label] = side[str_faction_name].t_str();
		...
		const int income_amt = side[str_income].to_int();
		if(income_amt != 0) {
		...
		if(select_leader_button) {
			if(side[str_player_id] == prefs::get().login() && side[str_allow_changes].to_bool(true)) {
				//
