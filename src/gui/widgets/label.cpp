...
label_definition::resolution::resolution(const config& cfg)
	...
	, link_color(cfg[str_link_color].empty() ? color_t::from_hex_string("ffff00") : color_t::from_rgba_string(cfg[str_link_color].str()))
{
...
builder_label::builder_label(const config& cfg)
	...
	, characters_per_line(cfg[str_characters_per_line])
