	...
	, fake_unit_hidden_(false)
{
	...
	unit_map::iterator unit_itor = resources::gameboard->units().find(cfg[str_unit_]);
	...
	route_->move_cost = route_cfg[str_move_cost];
	for(const config& loc_cfg : route_cfg->child_range(str_step)) {
		route_->steps.emplace_back(loc_cfg[str_x],loc_cfg[str_y], wml_loc());
	}
	for(const config& mark_cfg : route_cfg->child_range(str_mark)) {
		route_->marks[map_location(mark_cfg[str_x],mark_cfg[str_y], wml_loc())]
			= pathfind::marked_route::mark(mark_cfg[str_turns],
