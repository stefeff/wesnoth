...
{
	gold = cfg[str_gold].to_int();
	income = cfg[str_income].to_int();
	team_name = cfg[str_team_name].str(cfg[str_side].str());
	user_team_name = cfg[str_user_team_name].t_str();
	side_name = cfg[str_side_name].t_str();
	faction = cfg[str_faction].str();
	faction_name = cfg[str_faction_name].t_str();
	id = cfg[str_id].str();
	save_id = cfg[str_save_id].str(id);
	current_player = cfg[str_current_player].str();
	countdown_time = cfg[str_countdown_time].str();
	action_bonus_count = cfg[str_action_bonus_count].to_int();
	flag = cfg[str_flag].str();
	flag_icon = cfg[str_flag_icon].str();
	scroll_to_leader = cfg[str_scroll_to_leader].to_bool(true);
	objectives = cfg[str_objectives].t_str();
	...
	start_gold = cfg[str_start_gold].to_int(gold);
...
void team::build(const config& cfg, const gamemap& map)
{
	...
	countdown_time_ = chrono::parse_duration<std::chrono::milliseconds>(cfg[str_countdown_time]);
	action_bonus_count_ = cfg[str_action_bonus_count].to_int();
