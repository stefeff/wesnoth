...
void addon_info::read(const config& cfg)
{
	...
	size = cfg[str_size].to_int();
	downloads = cfg[str_downloads].to_int();
	uploads = cfg[str_uploads].to_int();
	...
	updated = chrono::parse_timestamp(cfg[str_timestamp]);
	created = chrono::parse_timestamp(cfg[str_original_timestamp]);
...
void addon_info::write(config& cfg) const
{
	...
	cfg[str_timestamp] = chrono::serialize_timestamp(updated);
	cfg[str_original_timestamp] = chrono::serialize_timestamp(created);
}
