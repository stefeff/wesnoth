...
location_set teleport_map::get_adjacents(map_location loc) const
{
...
location_set teleport_map::get_sources() const
{
	location_set res;
...
location_set teleport_map::get_targets() const
{
	location_set res;
	...
	bool see_all, bool ignore_units, bool check_vision)
{
	...
	for (const unit_ability & teleport : u.get_abilities("teleport")) {
		const int tunnel_count = (teleport.ability_cfg)->child_count(str_tunnel);
		for(int i = 0; i < tunnel_count; ++i) {
			config teleport_group_cfg = (teleport.ability_cfg)->mandatory_child(str_tunnel, i);
