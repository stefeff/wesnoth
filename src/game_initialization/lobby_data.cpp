...
game_info::game_info(const config& game, const std::vector<std::string>& installed_addons)
	: id(game[str_id].to_int())
	, map_data(game[str_map_data])
	, name(font::escape_text(game[str_name].str()))
	, scenario()
	, scenario_id()
	, type_marker()
	...
	, game_preset(game[str_game_preset].to_bool())
	...
	, addons_outcome(addon_req::SATISFIED)
{
	...
	for(const config& addon : game.child_range("addon")) {
		if(addon.has_attribute("id") && addon[str_required].to_bool(false)) {
			if(!utils::contains(installed_addons, addon[str_id].str())) {
				required_addon r;
	...
	if(!game[str_mp_scenario].empty() && game[str_mp_campaign].empty()) {
		...
		if(level_cfg) {
			...
			scenario_id = (*level_cfg)[str_id].str();
			...
			if(!reloaded) {
				if(auto hashes = game_config.optional_child("multiplayer_hashes")) {
					std::string scenario = game[str_mp_scenario];
					if(hashes[scenario] != game[str_hash]) {
