...
void save_index_class::rebuild(const std::string& name, const std::chrono::system_clock::time_point& modified)
{
	...
	summary[str_mod_time] = chrono::serialize_timestamp(modified);
	write_save_index();
}
...
{
	config& root = data();
	root.remove_children("save", [&name](const config& d) { return d[str_save] == name; });
	write_save_index();
}
...
void save_index_class::clean_up_index()
{
	...
	if(root.all_children_count() > filenames.size()) {
		root.remove_children(str_save, [&filenames](const config& d)
			{
				return !utils::contains(filenames, d[str_save].str());
			}
		);
	}
}
