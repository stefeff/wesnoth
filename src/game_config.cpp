...
void load_config(const config &v)
{
	...
	lobby_refresh    = chrono::parse_duration(v[str_lobby_refresh], 2000ms);
	...
	if(auto i = v.optional_child(str_colors)){
		...
		reach_map_color       = i[str_reach_map_color].str();
		reach_map_enemy_color = i[str_reach_map_enemy_color].str();
	} // colors
	...
	if(auto i = v.optional_child(str_images)){
		...
		if (!i[str_game_title_background].blank()) {
			// Select a background at random
			const auto backgrounds = utils::split(i[str_game_title_background].str());
	...
	reach_map_prefix 	= v[str_reach_map_prefix].str();
	reach_map_border_opacity = v[str_reach_map_border_opacity].to_int(100);
	reach_map_tint_opacity   = v[str_reach_map_tint_opacity].to_int(50);//tint is at 50% by default instead of 100% to allow players to make it more opaque than normal
