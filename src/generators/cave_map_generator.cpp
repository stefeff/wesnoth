...
cave_map_generator::cave_map_generator(const config &cfg) :
	...
	cfg_(cfg),
	width_(cfg_[str_map_width].to_int(50)),
	height_(cfg_[str_map_height].to_int(50)),
	village_density_(cfg_[str_village_density].to_int(0)),
	flipx_chance_(cfg_[str_flipx_chance].to_int()),
	flipy_chance_(cfg_[str_flipy_chance].to_int())
{
}
...
void cave_map_generator::cave_map_generator_job::generate_chambers()
{
	for (const config &ch : params.cfg_.child_range(str_chamber))
	{
		...
		int jagged_edges = ch[str_jagged].to_int();
...
void cave_map_generator::cave_map_generator_job::place_chamber(const chamber& c)
{
	...
	for(const auto [child_key, child_cfg] : c.items->all_children_view())
	{
		...
		if (!child_cfg[str_same_location_as_previous].to_bool()) {
			index = rng_()%c.locs.size();
		}
		std::string loc_var = child_cfg[str_store_location_as];
		...
		if (child_key == "side" && !child_cfg[str_no_castle].to_bool()) {
			place_castle(child_cfg[str_side].to_int(-1), *loc);
		}
...
void cave_map_generator::cave_map_generator_job::place_passage(const passage& p)
{
	...
	int windiness = p.cfg[str_windiness].to_int();
	...
	int jagged = p.cfg[str_jagged].to_int();
