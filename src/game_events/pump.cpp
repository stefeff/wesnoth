...
void wml_event_pump::process_event(handler_ptr& handler_p, const queued_event& ev)
{
	...
	scoped_weapon_info first_weapon("weapon", ev.data.optional_child(str_first));
	scoped_weapon_info second_weapon("second_weapon", ev.data.optional_child(str_second));
