...
static config expand_partialresolution(const config& theme)
{
	...
	for(const auto& part : theme.child_range(str_partialresolution)) {
		...
		for(const auto& add : part.child_range(str_add)) {
	...
	, font_rgb_(DefaultFontRGB)
{
	...
	if(cfg.has_attribute(str_font_rgb)) {
		font_rgb_ = color_t::from_rgb_string(cfg[str_font_rgb]);
		font_rgb_set_ = true;
	}
}
	...
	, items_()
{
	...
	const auto& cmd = hotkey::get_hotkey_command(items_[0][str_id]);
...
bool theme::set_resolution(const SDL_Rect& screen)
{
	...
	for(const config& i : cfg_.child_range(str_resolution)) {
		int width = i[str_width];
		int height = i[str_height];
	...
	cur_spec_width_ = (*current)[str_width];
	cur_spec_height_ = (*current)[str_height];
...
void theme::add_object(std::size_t sw, std::size_t sh, const config& cfg)
{
	...
	if(const auto status_cfg = cfg.optional_child(str_status)) {
