...
void saved_game::expand_mp_events()
{
	...
	if(starting_point_type_ == starting_point::SCENARIO && !starting_point_[str_has_mod_events].to_bool(false)) {
		...
		while(starting_point_.has_child(str_load_resource)) {
			assert(starting_point_.child_count(str_load_resource) > 0);
			std::string id = starting_point_.mandatory_child(str_load_resource)[str_id];
			size_t pos = starting_point_.find_total_first_of(str_load_resource);
			starting_point_.remove_child(str_load_resource, 0);
...
{
	config& map_scenario = map_scen.has_child(str_multiplayer) ? map_scen.mandatory_child(str_multiplayer) : (map_scen.has_child(str_scenario) ? map_scen.mandatory_child(str_scenario) : map_scen);
	config sides;
...
void saved_game::update_label()
{
	...
	if(classification().abbrev.empty()) {
		...
		label = classification().abbrev + "-" + starting_point_[str_name];
	}
...
void saved_game::set_data(config& cfg)
{
	...
	mp_settings_ = { cfg.child_or_empty(str_multiplayer) };
