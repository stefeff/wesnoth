...
{
	std::string campaign_id = load_data.summary[str_campaign];
	...
	if(const auto campaign = game_config.find_child("campaign", "id", campaign_id)) {
		...
		load_data.load_config[str_difficulty] = load_data.difficulty;
...
{
	return check_version_compatibility(cfg[str_version].str());
}
